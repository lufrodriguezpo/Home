...
var $MargenDerechaWord;	 // Margen derecha para WordText hecha po ABC
...
function _parsejpg($file)
{
	...
	if($a[2]!=2 && $a[2]!=3){
		print_r($a);
		echo 'Not a (JPEG/JPG) file: '.$file.'<hr>';
	}
...
function WriteText ( $text , $h=0) {
	global $SetLeft,$SetRight;
	if($h==0) $h=5; 
	$intPosIni = 0 ;
	$intPosFim = 0 ;
	if(!empty($SetLeft) && !empty($SetRight)){
		$this->SetLeftMargin($SetLeft);	
		if ($this->MargenDerechaWord>0) $SetRight=$this->MargenDerechaWord;
		$this->SetRightMargin($SetRight);
	}
	else{
	...
	if ( strpos ( $text , '<' )!== false && strpos ( $text , '[' )!== false ) { 
		if ( strpos ( $text , '<' )< strpos ( $text , '[' )) { 
			...
			$this->SetFont( '' , '' ); 
			$this->WriteText( substr ( $text , $intPosFim + 1 , strlen ( $text )), $h); 
		} else { 
			$this->Write( $h , substr ( $text , 0 , strpos ( $text , '[' ))); 
			$intPosIni = strpos ( $text , '[' ); 
			$intPosFim = strpos ( $text , ']' ); 
		...
		if ( strpos ( $text , '<' )!== false ) { 
			...
			$intPosFim = strpos ( $text , '>' ); 
			$this->SetFont ( '' , 'B' ); 
			$this->WriteText ( substr ( $text , $intPosIni + 1 , $intPosFim - $intPosIni - 1 ), $h); 
			$this->SetFont ( '' , '' ); 
			$this->WriteText ( substr ( $text , $intPosFim + 1 , strlen ( $text )), $h); 
