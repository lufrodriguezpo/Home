...
$Agendadas = array();
$consulta="SELECT * FROM citasambientes WHERE anio='$anio' AND sems='$sems' AND codarea = '$codareaE' AND codalu='$codigoU'";
$resultado=$mysqli->query($consulta) or die("Connection failed: " . $mysqli->error.' - '.$mysqli->close().'Ok.');
while($registro = $resultado->fetch_assoc()){
	$mes = (int)$registro['mes'];
	$dia = (int)$registro['dia'];
	$hora = (int)$registro['hora'];
	$Agendadas[$mes.'-'.$dia.'-'.$hora] = 'S';
	$Agendadas[$mes.'-'.$dia] = 'S';
	$Agendadas[$mes] = 'S';
}
...
<link rel="stylesheet" type="text/css" href="estilosInscribirAmbiente.css?v=1.3"/>
<script>
...
function DialogoAsignada(element){
	...
	html += '</table>';		
	if(agendada!="si"){
		var buttons = [{text: "Inscribirme", click: function() {Inscribirse(mes,dia,hora);}},
						{text: "Regresar", click: function() {$(this).dialog("close");}}];
		AbreDialog(html,"Inscribirse a Tutoria?","",buttons);
	}else{
		var buttons = [{text: "Cancelar Inscripcion", click: function() {CancelarInscripcion(mes,dia,hora);}},
						{text: "Regresar", click: function() {$(this).dialog("close");}}];
		AbreDialog(html,"Inscribirse a Tutoria?","",buttons);
	}
}
function CancelarInscripcion(mes,dia,hora){
	var datos = {};
	...
	datos['pin'] = validaD;
	$.ajax({
		url: "CancelarInscripcion.php",
		type: 'POST',
		async: 'true',
		data: datos
	}).done(function(respuesta){
		if(respuesta.substr(-2,2)=='Ok'){
			Agendadas[parseInt(mes)+'-'+parseInt(dia)+'-'+parseInt(hora)]='N';
			Agendadas[parseInt(mes)+'-'+parseInt(dia)]='N';
			Agendadas[parseInt(mes)]='N';			
			ArmarCalendario(anio,diaActual.getMonth(),diaActual.getDate());
			ArmarSemana();
			AbreDialog('La asignación se cancelo correctamente','Guardando información...','','');
...
function Inscribirse(mes,dia,hora){
	var datos = {};
	datos['mes'] = $("body").encodeUrl(mes);
	datos['dia'] = $("body").encodeUrl(dia);
	datos['hora'] = $("body").encodeUrl(hora);
	...
	datos['pin'] = validaD;
	$.ajax({
		url: "GuardaInscripcion.php",
		type: 'POST',
		async: 'true',
		data: datos
	}).done(function(respuesta){
		if(respuesta.substr(-2,2)=='Ok'){
			Agendadas[parseInt(mes)+'-'+parseInt(dia)+'-'+parseInt(hora)]='S';
			Agendadas[parseInt(mes)+'-'+parseInt(dia)]='S';
			Agendadas[parseInt(mes)]='S';
			ArmarCalendario(anio,diaActual.getMonth(),diaActual.getDate());
			ArmarSemana();
