...
$resultado=$mysqli->query($consulta) or die("Connection failed: " . $mysqli->error.' - '.$mysqli->close().'Ok.');
while($registro = $resultado->fetch_assoc()){
	$codarea = 'A'.$registro['codarea'];
	$Areas[$codarea] = $registro['area'];
}
...
$(document).ready(function(){
	...
	$("body").on("click","#perfil_foto_anuncio",function(){
		window.location.href =  "../ModuloEstudiante/Perfil.php";
	});
...
function AbrirOpcion(element){
	...
	if(tipo ==1){
		var mensa = 'Por favor, seleccione un area para continuar el proceso:<br>';
		mensa +='<table>';
		var na = 0;
		...
		mensa +='<table>';
		if(na==0){
			mensa = 'No esta habilitada esta opcion en el momento.';	
		}
