...
$Horarios = array();
$consulta="SELECT * FROM horariospares WHERE anio='$anio' AND sems='$sems' AND codigo='$codigoU' AND codarea = '$codareaE'";
$resultado=$mysqli->query($consulta) or die("Connection failed: " . $mysqli->error.' - '.$mysqli->close().'Ok.');
while($registro = $resultado->fetch_assoc()){
...
<link rel="stylesheet" type="text/css" href="estilosMonitor.css?v=1.3"/>
<link rel="stylesheet" type="text/css" href="estilosCalendario.css?v=1.4"/>
<link rel="stylesheet" type="text/css" href="estilosAsignarHorariosPares.css?v=1.7"/>
<script>
...
var codigoM = '<?PHP echo $_SESSION['codigoUsuario'];?>';
...
function GuardarAsignacion(){
	...
	var datos = {};
	$(".contenedor_calendario_hora.horenable").each(function(){
		var val = "N";
		if($(this).attr("selected")=="selected")val = "S";
		var id = "VF"+parseInt($(this).data("mes"))+"-"+parseInt($(this).data("dia"))+"-"+parseInt($(this).data("hora"));
		Horarios[id.substr(2)]= val;
		if(val=='S')Horarios[parseInt($(this).data("mes"))+"-"+parseInt($(this).data("dia"))]= val;
		if(val=='S')Horarios[parseInt($(this).data("mes"))]= val;
		datos[id]=val;
	});
	datos['pin']=validaD;
...
function ArmarSemana(){
	...
	$.each(semana,function(inds,nds){
		...
		$.each(rangoHora,function(ind,val){
			...
			if(selec=='S')selec = 'selected="selected"';
			else selec = '';
			html+='<div class="contenedor_calendario_hora eDiaSemana horenable" data-dia="'+diasMes[inds][0]+'" data-hora="'+val+'" data-mes="'+diasMes[inds][1]+'" '+selec+'></div>';
		});
		html += '</div>';
	});
	$("#semanas").html(html);
...
function toggleSeleccion(cell){
	...
	else cell.attr("selected","selected");
}
	...
	<div id="contenedor-body">
		...
		<section id="contenedor_todo">
			...
			<div id="botones" style="display:none">
				<button>Guardar Asignacion</button>
			</div>
			</div>		
			<br />
			</div>
