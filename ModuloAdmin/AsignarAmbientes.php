...
$Horarios = array();
$InfoCita = array();
$consulta="SELECT * FROM horariosambientes WHERE anio='$anio' AND sems='$sems' AND codarea = '$codareaE'";
$resultado=$mysqli->query($consulta) or die("Connection failed: " . $mysqli->error.' - '.$mysqli->close().'Ok.');
while($registro = $resultado->fetch_assoc()){
...
<title>.:Calendario Monitor:.</title>
...
<link rel="stylesheet" type="text/css" href="estilosAdmin.css?v=1.2"/>
<link rel="stylesheet" type="text/css" href="estilosCalendario.css?v=1.2"/>
<link rel="stylesheet" type="text/css" href="estilosCardPerfil.css?v=1.1"/>
<link rel="stylesheet" type="text/css" href="estilosAsignarAmbientes.css?v=1.5"/>
<script>
...
$(document).ready(function(){
	...
	$("body").on("mouseup","#contenedor-body",function(){
		if(mousePress==1)AsignarCarga();
		mousePress = 0;
	});
	
	$("body").on("click",".horenable",function(){
...
function toggleSeleccion(cell){
	...
	var asig =  cell.attr("asignada");
	if(sel !="selected" && asig!="asignada"){
		...
		var html ='';
		html +='<table>';
		html +='<tr><td  style="padding-bottom:20px">Lugar:</td><td  style="padding-bottom:20px"><input type="text" id="lugar" style="width:250px;"></td></tr>';
		if(Monitores != null)$.each(Monitores,function(ind,monitor){
			if(ind!=0){
				html +='<tr><td>';
				html += '<input type="checkbox" value="'+ind+'" id="MON'+ind+'" class="selecionMon">';
				html += '</td><td>';
		...
		html +='</table>';
		var buttons = [{text: "Editar", click: function() {GuardarAsignacion();$(this).dialog("close");}},
						{text: "Eliminar", click: function() {EliminarAsignacion();}},
						{text: "Regresar", click: function() {DeseleccionarTodas();$(this).dialog("close");}}];
		AbreDialog(html,"Asignar Monitores al horario",'',buttons);
	}
}
function AsignarCarga(){
	var html ='';
	var n =0 ;
	$(".horenable[selected=selected]").each(function(){
		n++;	
	});
	if(n==0)return 0;
	html +='<table>';
	...
	html +='</table>';
	var buttons = [{text: "Cargar", click: function() {GuardarAsignacion();}},
					{text: "Regresar", click: function() {DeseleccionarTodas();$(this).dialog("close");}}];
	AbreDialog(html,"Asignar Monitores al horario",'',buttons);
}
function GuardarAsignacion(){
	var mnt = new Array();
	var lugar = $("#lugar").val();
	$(".selecionMon:checked").each(function(){
		var value=$(this).attr("value");
		mnt[mnt.length]= value;
	});
	mnt = mnt.join();
	if(mnt==''){
		alert("Es obligario asignar por lo menos un monitor!");
		return 0;
	}
	if(lugar==''){
		alert("Es obligario asignar un lugar!");
		return 0;
	}
	var fechas = new Array();
	$(".horenable[selected=selected]").each(function(){
		$(this).attr("selected",false);
		$(this).attr("asignada","asignada");
		$(this).data("carga","si");
		$(this).data("monitores",mnt);
		$(this).data("lugar",lugar);
		var fecha = parseInt($(this).data("mes"))+'-'+parseInt($(this).data("dia"))+'-'+parseInt($(this).data("hora"));
		fechas[fechas.length] = fecha;
		Horarios[fecha]= 'S';
		Horarios[parseInt($(this).data("mes"))+"-"+parseInt($(this).data("dia"))]= 'S';
		Horarios[parseInt($(this).data("mes"))]= 'S';
		InfoCita[fecha]= new Array(mnt,lugar);
	});
	fechas = fechas.join();
	GuardarFecha(fechas,lugar,mnt);
	$("#dialogo").dialog("close");
}
function GuardarFecha(fechas,lugar,mons){
	var datos = {};
	datos['fechas'] = $("body").encodeUrl(fechas);
	datos['lugar'] = $("body").encodeUrl(lugar);
	datos['monitores'] = $("body").encodeUrl(mons);
	datos['codarea'] =codarea;
	datos['pin'] = validaD;
	$.ajax({
		url: "GuardarHorariosAmbientes.php",
		type: 'POST',
		async: 'true',
		data: datos
		...
		if(respuesta.substr(-2,2)=='Ok'){
			ArmarCalendario(anio,diaActual.getMonth(),diaActual.getDate());	
			ArmarSemana();
			AbreDialog('La asignación se guardo correctamente','Guardando información...','','');
		}else if(respuesta.substr(-5,5)=='Error' && respuesta.substr(-7,1)==1){
			AbreDialog('No se encontro en la sesión el codigo de monitor, por favor, recargue la pagina e intente nuevamente.','Error al Guardar!','#FF0000','');
		}else if(respuesta.substr(-5,5)=='Error' && respuesta.substr(-7,1)==2){
			AbreDialog('Ha ocurrido un error al momento de guardar, por favor recargue la pagina.','Error al Guardar!','#FF0000','');
		}else{
			AbreDialog('Ha ocurrido un error al momento de agregar al tutor, por favor, recargue la pagina  e intente de nuevo.','Error al Guardar!','#FF0000','');
		}
	});
}
function EliminarAsignacion(){
	var eliminadas = new Array();
	$(".horenable[selected=selected]").each(function(){
		$(this).attr("selected",false);
		$(this).attr("asignada","null");
		$(this).data("carga","no");
		$(this).data("monitores","null");
		var fecha = parseInt($(this).data("mes"))+'-'+parseInt($(this).data("dia"))+'-'+parseInt($(this).data("hora"));
		eliminadas[eliminadas.length]= fecha;
		Horarios[fecha]= 'N';
		//Horarios[parseInt($(this).data("mes"))+"-"+parseInt($(this).data("dia"))]= 'N';
		//Horarios[parseInt($(this).data("mes"))]= 'N';
	});
	eliminadas = eliminadas.join();
	CancelarFecha(eliminadas);
	$("#dialogo").dialog("close");
}
function CancelarFecha(fechas){
	var datos = {};
	datos['fechas'] = $("body").encodeUrl(fechas);
	datos['codarea'] =codarea;
	datos['pin'] = validaD;
	$.ajax({
		url: "CancelarHorariosAmbientes.php",
		type: 'POST',
		async: 'true',
		data: datos
	}).done(function(respuesta){
...
function ArmarSemana(){
	...
	$.each(semana,function(inds,nds){
		...
		$.each(rangoHora,function(ind,val){
			...
			html+='<div title="'+titleD+'" class="contenedor_calendario_hora eDiaSemana horenable" data-dia="'+diasMes[inds][0]+'" data-hora="'+val+'" data-mes="'+diasMes[inds][1]+'" '+selec+'>';
			html+=info+'</div>';
		});
		html += '</div>';
	});
...
function Seleccionar(cellI,cellF){
	...
	var rangoC = numerosRango(cellI[0],cellF[0],1);
	DeseleccionarTodas();
	$.each(rangoC,function(ind,valC){
		$.each(rangoH,function(ind,val){
			$(".horenable[data-dia="+valC+"][data-hora="+val+"]").attr("selected","selected");
			$(".horenable[data-dia="+valC+"][data-hora="+val+"]").data("carga","no");
		});
	});
}
